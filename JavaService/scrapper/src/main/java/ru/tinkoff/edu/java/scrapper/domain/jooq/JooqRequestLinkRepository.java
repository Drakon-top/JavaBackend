...
public class JooqRequestLinkRepository {
    ...
    private ZoneOffset zoneOffset = ZoneOffset.ofHours(TIME_OFFSET);
    ...
    public List<DataLinkWithInformation> findLinkNotUpdateLongTime(int countLink) {
        return dslContext.selectFrom(Link.LINK)
            .fetch().stream().map(x -> {
                try {
                    return new DataLinkWithInformation(Long.valueOf(x.getId()),
                        new URI(x.getUrl()),
                        x.getLastUpdate().atOffset(zoneOffset),
                        x.getLastEditTime().atOffset(zoneOffset),
                        x.getCountCommitOrQuestion()
                    );
                } catch (URISyntaxException e) {
                    throw new RuntimeException(e);
                }
